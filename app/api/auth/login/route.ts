  ...
  try {
    ...
    if (!its_no_raw || !paci_no) {
      return NextResponse.json(
        { error: "ITS No and PACI No are required" },
        { status: 400 },
      );
    }
    ...
    // ✅ Single query from the view
    const { data: loginRow, error } = await supabase
      .from("mumin_login")
      .select("its_no, supabase_auth_id, paci_no")
      .eq("its_no", its_no_num)
      .maybeSingle();

    if (error) {
      console.error("[login] view lookup error:", error);
      return NextResponse.json(
    ...
    const row = loginRow as MuminLoginRow | null;
    ...
    if (row.paci_no !== paci_no) {
      return NextResponse.json({ error: "Invalid PACI number" }, { status: 401 });
    }

    if (!row.supabase_auth_id) {
      return NextResponse.json(
        { error: "Account not provisioned. Contact your Masool." },
        { status: 401 },
      );
    }
