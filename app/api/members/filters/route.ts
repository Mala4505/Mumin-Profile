...
//   // Fetch sectors (scoped for Masool)
...
//   // Fetch subsectors with Musaid names via RPC (with fallback)
//   const { data: rpcData, error: rpcError } = await supabase.rpc(
//     "get_subsectors_with_musaid",
//   );

//   let subsectorsBase: Array<{
//     subsector_id: number;
...
//   }>;
//   if (rpcError || !rpcData) {
//     // Fallback: plain subsector query if RPC doesn't exist yet
//     const { data } = await supabase
//       .from("subsector")
//       .select("subsector_id, subsector_name, sector_id")
//       .order("subsector_id");
//     subsectorsBase = (data ?? []).map((s) => ({ ...s, musaid_names: null }));
//   } else {
//     subsectorsBase = rpcData;
//   }

//   // Apply filters and scoping to subsectors
//   let subsectors = subsectorsBase;
//   if (sectorId)
//     subsectors = subsectors.filter((s) => s.sector_id === Number(sectorId));
//   if (session.role === "Masool" && session.sector_ids?.length) {
//     subsectors = subsectors.filter((s) =>
//       session.sector_ids.includes(s.sector_id),
//     );
//   }
//   if (session.role === "Musaid" && session.subsector_ids?.length) {
//     subsectors = subsectors.filter((s) =>
//       session.subsector_ids.includes(s.subsector_id),
//     );
//   }

//   // Fetch musaids for dropdown — scoped by role
//   let musaidsQuery = adminClient
//     .from("user_subsector")
//     .select("its_no, mumin!inner(name)");

//   if (session.role === "Masool" && session.sector_ids?.length) {
//     const visibleSubsectorIds = subsectors.map((s) => s.subsector_id);
//     if (visibleSubsectorIds.length > 0) {
//       musaidsQuery = musaidsQuery.in("subsector_id", visibleSubsectorIds);
//     }
...
//   // Deduplicate by its_no (a musaid may cover multiple subsectors)
//   const musaidMap = new Map<number, string>();
//   for (const row of (musaidRows ?? []) as any[]) {
//     const itsNo = row.its_no as number;
//     if (!musaidMap.has(itsNo)) {
//       musaidMap.set(itsNo, row.mumin?.name ?? "");
//     }
//   }
...
//   // Expand subsectors: one row per musaid so each gets its own dropdown option
//   const expandedSubsectors: Array<{
//     subsector_id: number;
//     subsector_name: string;
//     sector_id: number;
...
//           subsector_name: ss.subsector_name,
//           sector_id: ss.sector_id,
//           musaid_name: name.trim() as string | null,
//         }));
//     })
//     .sort((a, b) => a.subsector_id - b.subsector_id); // ← keep order after expansion

//   const { data: sectors } = await sectorsQuery;

//   return NextResponse.json({
//     sectors: sectors ?? [],
//     subsectors: expandedSubsectors,
//     musaids,
//   });
// }
