...
export async function GET() {
  ...
  if (['Masool', 'Musaid'].includes(session.role)) {
    // See forms they created OR are listed as a filler
    query = query.or(`created_by.eq.${Number(session.its_no)},filler_access->fillers.cs.[{"type":"specific_masool"},{"type":"specific_musaid"}]`)
  }
  ...
  if (formIds.length) {
    const { data: rCounts } = await supabase
      .from('form_responses') // pluralized
      .select('form_id')
      .in('form_id', formIds)
      .eq('submitted', true)
...
export async function POST(req: NextRequest) {
  ...
  // Questions are stored in form_fields — never in forms.questions JSONB.
  const { data, error } = await supabase
    .from('forms')
    .insert({
      title: body.title,
