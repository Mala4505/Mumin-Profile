...
export async function getSuperAdminStats(): Promise<SuperAdminStats> {
  ...
  const [muminResult, sectorResult, subsectorResult, userResult, activityResult, buildingResult, houseResult] = await Promise.all([
  ...
  const sectorStats = await Promise.all(
    sectors.map(async (s) => {
      ...
      const sectorSubsectorIds = new Set(
        (await supabase.from('subsector').select('subsector_id').eq('sector_id', s.sector_id)).data?.map((ss: any) => ss.subsector_id) ?? []
      )
      const sectorBuildings = allBuildings.filter(b => sectorSubsectorIds.has(b.subsector_id))
      ...
      return {
        ...
        subsector_count: ssCount.count ?? 0,
...
export async function getMusaidStats(session: SessionUser): Promise<MusaidStats> {
  ...
  const subsectorId = subsectorIds[0] // Primary for display name
