...
export interface MasoolStats {
  ...
  totalFlats: number
  totalFamilies: number
  subsectors: Array<{ subsector_id: number; subsector_name: string; mumin_count: number; sector_name: string }>
  buildings: Array<{
    building_id: number
...
export async function getSuperAdminStats(): Promise<SuperAdminStats> {
  ...
  const [
    muminResult, sectorResult, subsectorResult, userResult, activityResult,
    buildingResult, houseResult,
    masoolSectorRows, musaidSubsectorRows, masoolUsersResult, musaidUsersResult,
  ] = await Promise.all([
    ...
    supabase.from('activity_log').select('id, action, entity_type, performed_by_its, created_at').order('created_at', { ascending: false }).limit(8),
  ...
  const musaidUserMap = new Map<number, LeaderContact>(
    ((musaidUsersResult.data ?? []) as any[]).map((m: any) => [m.its_no, { its_no: m.its_no, name: m.name, phone: m.phone }])
  )
  ...
  for (const row of (masoolSectorRows.data ?? []) as Array<{ its_no: number; sector_id: number }>) {
    ...
    if (!contact) continue
    if (!masoolsBySector.has(row.sector_id)) masoolsBySector.set(row.sector_id, [])
    masoolsBySector.get(row.sector_id)!.push(contact)
  }

  const musaidsBySubsector = new Map<number, LeaderContact[]>()
  for (const row of (musaidSubsectorRows.data ?? []) as Array<{ its_no: number; subsector_id: number }>) {
    const contact = musaidUserMap.get(row.its_no)
    if (!contact) continue
    if (!musaidsBySubsector.has(row.subsector_id)) musaidsBySubsector.set(row.subsector_id, [])
  ...
  const sectorStats = await Promise.all(
    sectors.map(async (s) => {
      ...
      const subsectorsInSector = (ssSectorsResult.data ?? []) as Array<{ subsector_id: number; subsector_name: string }>
      const sectorSubsectorIds = subsectorsInSector.map(ss => ss.subsector_id)
      const sectorBuildings = allBuildings.filter(b => sectorSubsectorIds.includes(b.subsector_id))
      const sectorFlatCount = sectorBuildings.reduce((sum, b) => sum + (flatCountByBuilding.get(b.building_id) ?? 0), 0)
      ...
      const [mCountResult, ...subsectorDetails] = await Promise.all([
        sectorSubsectorIds.length > 0
          ? supabase.from('mumin').select('its_no', { count: 'exact' }).in('subsector_id', sectorSubsectorIds)
          : Promise.resolve({ count: 0 }),
        ...subsectorsInSector.map(async (ss) => {
          const { count } = await supabase.from('mumin').select('its_no', { count: 'exact' }).eq('subsector_id', ss.subsector_id)
          return {
      ...
      return {
        ...
        subsectors: subsectorDetails as Array<{ subsector_id: number; subsector_name: string; mumin_count: number; musaids: LeaderContact[] }>,
      }
    })
  )
...
export async function getAdminStats(session: SessionUser): Promise<AdminStats> {
  ...
  if (sectorIds.length === 0) {
    return {
      assignedSectors: [],
      totalSubsectors: 0, totalBuildings: 0, totalFlats: 0,
      totalFamilies: 0, totalMumineen: 0, sectorBreakdown: [],
    }
  }

  const [
    sectorsResult, subsectorsResult,
    masoolSectorRows, musaidSubsectorRows, masoolUsersResult, musaidUsersResult,
  ] = await Promise.all([
  ...
  const musaidsBySubsector = new Map<number, LeaderContact[]>()
  for (const row of ((musaidSubsectorRows.data ?? []) as Array<{ its_no: number; subsector_id: number }>).filter(r => subsectorIds.includes(r.subsector_id))) {
    const contact = musaidUserMap.get(row.its_no)
    if (!contact) continue
    if (!musaidsBySubsector.has(row.subsector_id)) musaidsBySubsector.set(row.subsector_id, [])
    musaidsBySubsector.get(row.subsector_id)!.push(contact)
  ...
  const [buildingsResult, muminResult] = await Promise.all([
    subsectorIds.length > 0
      ? supabase.from('building').select('building_id, subsector_id').in('subsector_id', subsectorIds)
      : Promise.resolve({ data: [] }),
    supabase.from('mumin').select('its_no', { count: 'exact' }).in('subsector_id', subsectorIds),
  ])

  const buildings = (buildingsResult.data ?? []) as Array<{ building_id: number; subsector_id: number }>
  ...
  for (const ss of subsectors) {
    if (!subsectorsBySector.has(ss.sector_id)) subsectorsBySector.set(ss.sector_id, [])
  ...
  const sectorBreakdown = await Promise.all(
    sectors.map(async (s) => {
      ...
      const sectorBuildings = buildings.filter(b => sectorSubsectors.some(ss => ss.subsector_id === b.subsector_id))
      const sectorHouses = houses.filter(h => sectorBuildings.some(b => b.building_id === h.building_id))

      const mCount = ssIds.length > 0
        ? await supabase.from('mumin').select('its_no', { count: 'exact' }).in('subsector_id', ssIds)
        : { count: 0 }
...
export async function getMusaidStats(session: SessionUser): Promise<MusaidStats> {
  ...
  if (subsectorIds.length === 0) {
    return {
      subsectorName: 'Unassigned', subsectorId: 0, sectorName: '',
      muminCount: 0, maleCount: 0, femaleCount: 0,
      baligCount: 0, ghairBaligCount: 0,
      totalBuildings: 0, totalFlats: 0, totalFamilies: 0, buildings: [],
      sector_masools: [], subsector_musaids: [],
    }
  }
  ...
  const [ssResult, muminResult, buildingsResult] = await Promise.all([
    supabase.from('subsector').select('subsector_name, sector:sector_id(sector_id, sector_name)').eq('subsector_id', subsectorId).single(),
    supabase.from('mumin').select('gender, balig_status').in('subsector_id', subsectorIds),
    supabase.from('building').select('building_id, building_name').in('subsector_id', subsectorIds),
  ])

  const ssData = ssResult.data as any
  const sectorId = ssData?.sector?.sector_id

  const [masoolSectorRows, masoolUsersResult, musaidSubsectorRows, musaidUsersResult] = await Promise.all([
    sectorId
      ? supabase.from('user_sector').select('its_no').eq('sector_id', sectorId)
      : Promise.resolve({ data: [] }),
    supabase.from('mumin').select('its_no, name, phone').in('role', ['Masool', 'Admin']),
    supabase.from('user_subsector').select('its_no, subsector_id').in('subsector_id', subsectorIds),
    supabase.from('mumin').select('its_no, name, phone').eq('role', 'Musaid'),
  ])
  ...
  const sector_masools = ((masoolSectorRows.data ?? []) as Array<{ its_no: number }>)
  ...
  const subsector_musaids = ((musaidSubsectorRows.data ?? []) as Array<{ its_no: number; subsector_id: number }>)
    .filter(r => { if (musaidSeen.has(r.its_no)) return false; musaidSeen.add(r.its_no); return true })
    .map(r => musaidUserMap.get(r.its_no))
    .filter((c): c is LeaderContact => !!c)

  const mumineen = (muminResult.data ?? []) as Array<{ gender: string; balig_status: string }>
  const buildings = (buildingsResult.data ?? []) as Array<{ building_id: number; building_name: string }>
  const buildingIds = buildings.map(b => b.building_id)
...
export async function getMuminStats(itsNo: number): Promise<MuminStats | null> {
  ...
  const muminResult = await (supabase
    .from('mumin')
    .select('name, its_no, sabeel_no, gender, balig_status, status, subsector_id, subsector!subsector_id(subsector_name, sector:sector_id(sector_name))')
    .eq('its_no', itsNo)
    .single() as any)
