...
export async function getMembers(filters: MemberFilters): Promise<MemberListItem[]> {
  ...
  // Step A: family → paci_no + head_its_no for all sabeel_nos in this result set
  const { data: familyData } = await adminClient
    .from('family')
    .select('sabeel_no, paci_no, head_its_no')
    .in('sabeel_no', uniqueSabeelNos)

  const sabeelToFamily = new Map<string, { paci_no: string; head_its_no: number | null }>()
  for (const f of (familyData ?? []) as any[]) {
    if (f.paci_no) sabeelToFamily.set(f.sabeel_no, { paci_no: f.paci_no, head_its_no: f.head_its_no ?? null })
  }

  const paciNos = [...new Set([...sabeelToFamily.values()].map(v => v.paci_no))]
  ...
  // Step C: merge into each member
  for (const m of members) {
    const family = sabeelToFamily.get(m.sabeel_no)
    const house = family?.paci_no ? houseMap.get(family.paci_no) : undefined
    m.head_its_no = family?.head_its_no ?? null
    if (house) {
      m.paci_no = house.paci_no
      m.floor_no = house.floor_no
      m.flat_no = house.flat_no
