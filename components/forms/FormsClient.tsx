...
interface Form {
  ...
  created_by: number
  created_at: string
  filler_access?: { fillers: FillerEntry[] }
...
const STATUS_CONFIG: Record<Form['status'], { label: string; className: string }> = {
  draft: {
    label: 'Draft',
...
/** Check if this user is an authorized filler for a form — mirrors server-side logic */
function canUserFill(
  ...
  for (const f of fillerAccess.fillers) {
    if (f.type === 'role' && f.value === role) return true
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 flex flex-col gap-3 hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {role !== 'Mumin' && (
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${typeCfg.className}`}>
            {typeCfg.label}
          </span>
        )}
      </div>

      {/* Response progress — staff/admin view only */}
      {role !== 'Mumin' &&
        (form.status === 'published' || form.status === 'expired' || form.status === 'closed') &&
        form.audience_count != null &&
        form.audience_count > 0 && (
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>Responses</span>
            <span className="font-medium text-foreground">
              {form.response_count ?? 0} / {form.audience_count}
            </span>
          </div>
          <div className="h-1.5 bg-muted rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, Math.round(((form.response_count ?? 0) / form.audience_count) * 100))}%`
              }}
            />
          </div>
          <p className="text-xs text-muted-foreground">
            {Math.round(((form.response_count ?? 0) / form.audience_count) * 100)}% complete
            {form.status === 'published' && ` · ${form.audience_count - (form.response_count ?? 0)} pending`}
          </p>
        </div>
      )}
      ...
      <div className="flex gap-2 mt-auto pt-1">
        {/* Pending approval: SuperAdmin approve/reject */}
        {onApprove && onReject ? (
          <>
            <button
          ...
          /* Mumin: self-fill action only */
          canSelfFill ? (
            <button
              onClick={() => { setNavigating(true); router.push(`/forms/${form.id}/self-fill`) }}
              disabled={navigating}
            ...
            >
              {navigating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FilePen className="w-3.5 h-3.5" />}
              {navigating ? 'Loading…' : 'Fill Form'}
            </button>
          ) : (
          ...
          <button
            onClick={() => { setNavigating(true); router.push(`/forms/${form.id}/fill`) }}
            disabled={navigating}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-colors disabled:opacity-70"
          >
            {navigating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FilePen className="w-3.5 h-3.5" />}
            {navigating ? 'Loading…' : 'Fill'}
          </button>
        ) : (
...
export function FormsClient({ role, itsNo }: FormsClientProps) {
  ...
  const tabs: TabDef[] = [
    { id: 'my-forms', label: role === 'Mumin' ? 'My Forms' : 'My Forms', icon: <FileText className="w-3.5 h-3.5" /> },
    ...(role === 'SuperAdmin' ? [{ id: 'pending-approval' as TabId, label: 'Pending Approval', icon: <Clock className="w-3.5 h-3.5" /> }] : []),
    ...(role === 'SuperAdmin' || role === 'Admin' ? [{ id: 'all-forms' as TabId, label: 'All Forms', icon: <AlertTriangle className="w-3.5 h-3.5 opacity-70" /> }] : []),
  ]
  ...
  const canCreateForms = role !== 'Mumin'
      ...
      <div className="space-y-3">
        ...
        {role !== 'Mumin' && (
          <div className="flex items-center gap-4 px-4 py-2.5 bg-muted/40 rounded-lg border border-border text-xs text-muted-foreground">
            ...
            <span className="text-border">·</span>
            <span><span className="font-semibold text-foreground">{tabForms.reduce((sum, f) => sum + (f.response_count ?? 0), 0)}</span> responses</span>
          </div>
        )}
      </div>
