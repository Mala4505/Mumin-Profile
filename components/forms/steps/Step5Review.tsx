...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <dt className="text-xs font-semibold uppercase tracking-wider text-muted-foreground min-w-32">
      {children}
    </dt>
  )
}
...
export function Step5Review({ draft, onDraftChange, onBack, onComplete, role }: Props) {
  ...
  async function handleSubmit() {
    ...
    setLoading(true)
    try {
      // Step 1: POST form metadata (no questions — they go to form_fields)
      ...
      // Update parent draft state with form ID
      if (form && form.id) {
        onDraftChange?.({ ...draft, id: form.id })
      }

      // Step 2: POST form_fields (questions) as relational rows
      if ((draft.questions ?? []).length > 0) {
        const fieldsRes = await fetch(`/api/forms/${form.id}/fields`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            fields: (draft.questions ?? []).map((q, idx) => ({
        ...
        if (!fieldsRes.ok) {
          ...
          throw new Error(body.error ?? `Failed to save form fields (${fieldsRes.status})`)
        }
      }

      // Step 3: PUT to update status (published or pending_approval)
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-5">
      ...
      <div className="space-y-0 divide-y divide-border border border-border rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-3 bg-background">
          <dl className="flex flex-col gap-2 sm:flex-row sm:gap-4">
            ...
            <SectionValue>
              {draft.title || <span className="text-muted-foreground italic">Not set</span>}
            </SectionValue>
          </dl>
        </div>
        ...
        {draft.expires_at && (
          <div className="px-4 py-3 bg-background">
            <dl className="flex flex-col gap-2 sm:flex-row sm:gap-4">
              ...
              <SectionValue>
                {new Date(draft.expires_at).toLocaleDateString('en-GB', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric',
                })}
                ...
                <ol className="space-y-1 list-decimal list-inside">
                  {draft.questions.map((q, i) => (
                    <li key={i} className="text-sm text-foreground">
                      {q.question_text || (
                        <span className="italic text-muted-foreground">Unnamed question</span>
                      )}
                    </li>
                  ))}
                </ol>
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${targetStatus === 'published'
