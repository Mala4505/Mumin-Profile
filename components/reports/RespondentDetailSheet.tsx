...
export function RespondentDetailSheet({
  ...
  // Keep the last respondent rendered during the close animation. This is the
  // "adjust state during render" pattern rather than a ref, so the value is
  // read safely during render (a ref would be, and was, a react-hooks/refs bug).
  const [lastPerson, setLastPerson] = useState<Respondent | null>(null)
  if (respondent && respondent !== lastPerson) setLastPerson(respondent)
  const person = respondent ?? lastPerson
        ...
        {person && (
          <>
            <SheetHeader className="pb-4 border-b border-border space-y-1">
              ...
              <SheetDescription className="sr-only">ITS {person.its_no}</SheetDescription>
              <MemberIdentity name={person.name} itsNo={person.its_no} size="lg" />
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 pt-1 text-xs text-muted-foreground">
                {person.gender && <GenderPill gender={person.gender} size="sm" />}
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <dl className="px-4 sm:px-6 divide-y divide-border">
                {questions.map((q) => {
